...
LLARS is actively used in the development of CAIA~\cite{steigerwald2025caia}, an AI-assisted support system for text-based psychosocial counselling.
Counselling researchers and developers collaboratively refined prompts for seven LLM-driven functionalities, iterating against real data until outputs met professional standards.
For automated subject line generation~\cite{steigerwald2025subjectline}, the complete pipeline was exercised: batch generation produced 253~subject lines across 11~LLMs, five professionals and an LLM evaluator rated them (1,518~assessments), and cross-model comparison directly informed the selection of a privacy-preserving open-source model.
This work also motivated two domain-specific evaluation types (Figure~\ref{fig:scenario-wizard}):
\emph{Mail Rating} for assessing AI-generated responses on counselling-specific dimensions, and \emph{Authenticity} for classifying threads as real or AI-generated with ground truth labels.

% TODO: Fill in actual interview details
Semi-structured interviews with $N$~practitioners ($X$~counselling researchers, $Y$~developers) highlighted that the collaborative editor eliminated version confusion from iterating prompts in shared documents, batch generation with cost tracking made systematic cross-model comparison feasible, and domain experts could set up and conduct evaluations independently.
...
LLARS is open-source; future work targets a controlled study and cross-domain validation.
