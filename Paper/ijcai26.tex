...
\begin{abstract}
We demonstrate LLARS (LLM Assisted Research System), an open-source platform that bridges the gap between domain experts and developers for building LLM-based systems.
...
Yet domain experts and developers typically work in isolation. Prompts are iterated in shared documents without version control and often tested against the wrong model or not at all. Outputs end up scattered across tools and evaluation remains ad hoc without structured coordination among assessors.
LLARS (LLM Assisted Research System) is an open-source tool that connects these three stages in a single platform (Figure~\ref{fig:pipeline}).
...
Batch generation addresses this by combining any set of prompts, models and data items into the Cartesian product of prompts\,$\times$\,models\,$\times$\,data items.
...
Our demo\footnote{Video: \url{https://youtu.be/FdG1nJ7OqE0}} walks attendees through the complete workflow, from collaborative prompt authoring through batch generation to evaluation with live agreement metrics and provenance analysis, using pre-built counselling scenarios that let attendees author prompts, trigger batch runs and evaluate outputs hands-on.
