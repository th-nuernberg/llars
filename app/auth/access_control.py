...
def require_scenario_membership(scenario_id: int, user) -> None:
    ...
    - LEBENDE ScenarioUsers-Einträge (OWNER, EVALUATOR, VIEWER)

    Wichtig: Soft-entfernte Mitglieder (membership_status=ARCHIVED) und
    abgelehnte Einladungen (invitation_status=REJECTED) gelten NICHT als
    Mitglieder — sonst behalten entfernte/abgelehnte User Zugriff. Gefiltert
    wird auf "lebend" via Negation (!= ARCHIVED / != REJECTED), passend zur
    Active-Member-Filterung im Rest der Codebase (vgl. scenario_manager_api).
    ...
    from db.models import (
        RatingScenarios, ScenarioUsers, MembershipStatus, InvitationStatus
    )
    ...
    if user_id:
        membership = ScenarioUsers.query.filter(
            ...
            ScenarioUsers.invitation_status != InvitationStatus.REJECTED
        ).first()
        if membership:
            return
...
def require_item_in_scenario(scenario_id: int, item_id: int) -> None:
    """
    Prüft, dass das Item/der Thread wirklich zu diesem Szenario gehört.

    Verhindert cross-scenario IDOR: ein Mitglied von Szenario A übergibt
    scenario_id=A (Membership ok) zusammen mit einer item_id, die zu Szenario B
    gehört, und liest so fremde Item-Inhalte (Nachrichten/PII). Items können in
    mehreren Szenarien sein — es genügt, dass das Item im ABGEFRAGTEN Szenario
    liegt. Beide Verknüpfungstabellen werden geprüft (Kompatibilität).

    Raises:
        NotFoundError: Item gehört nicht zu diesem Szenario.
    """
    from db.models import ScenarioThreads

    if ScenarioThreads.query.filter_by(scenario_id=scenario_id, thread_id=item_id).first():
        return
    try:
        from db.models import ScenarioItems
        if ScenarioItems.query.filter_by(scenario_id=scenario_id, item_id=item_id).first():
            return
    except Exception:
        pass
