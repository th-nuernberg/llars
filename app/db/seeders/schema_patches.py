...
def apply_schema_patches(db) -> None:
    ...
    try:
        ...
        # Conference Manager: series_id FK on conferences
        changed |= _ensure_column(
            db,
        ...
        # Conference Manager: latex_workspace_id FK on papers
        changed |= _ensure_column(
            db,
            table_name="papers",
        ...
        # Migrate messaging → communication permission keys
        changed |= _migrate_messaging_permissions(db)
...
def _migrate_messaging_permissions(db) -> bool:
    """
    Idempotent migration: rename old messaging permission keys to new communication keys.

    Maps:
      feature:messaging:view → feature:communication:access
      feature:messaging:call → (removed, split into voice/video)
      feature:messaging:ai   → feature:communication:ai

    This updates the `permissions` table only. Role/user assignments will be
    synced by the permission seeder which adds/removes based on target lists.
    """
    rename_map = {
        'feature:messaging:view': 'feature:communication:access',
        'feature:messaging:ai': 'feature:communication:ai',
    }
    remove_keys = ['feature:messaging:call']
    ...
    try:
        ...
        for old_key, new_key in rename_map.items():
            ...
            old_exists = db.session.execute(
                text("SELECT COUNT(*) FROM permissions WHERE permission_key = :k"),
                {"k": old_key},
            ).scalar()
            ...
            if old_exists and not new_exists:
                db.session.execute(
                    text("UPDATE permissions SET permission_key = :new_key WHERE permission_key = :old_key"),
                    {"old_key": old_key, "new_key": new_key},
                )
                changed = True
                print(f"  [Permission Migration] {old_key} → {new_key}")
            elif old_exists and new_exists:
                # New already exists (from seeder), just delete the old one and migrate references
                old_id = db.session.execute(
                    text("SELECT id FROM permissions WHERE permission_key = :k"),
                    {"k": old_key},
                ).scalar()
                ...
                if old_id and new_id:
                    ...
                    db.session.execute(
                        text("DELETE FROM role_permissions WHERE permission_id = :old_id"),
                        {"old_id": old_id},
                    )
                    # Migrate user_permissions
                    if _table_exists(db, 'user_permissions'):
                        db.session.execute(
                            text("DELETE FROM user_permissions WHERE permission_id = :old_id"),
                            {"old_id": old_id},
                        )
                    db.session.execute(
                        text("DELETE FROM permissions WHERE id = :old_id"),
                        {"old_id": old_id},
                    )
                    changed = True

        # Remove obsolete permission keys
        for key in remove_keys:
            ...
            if exists:
                ...
                if _table_exists(db, 'user_permissions'):
                    db.session.execute(
                        text("DELETE FROM user_permissions WHERE permission_id = :pid"),
                        {"pid": exists},
                    )
                db.session.execute(
                ...
                changed = True
                print(f"  [Permission Migration] Removed {key}")

        if changed:
            db.session.commit()
    except Exception as exc:
        db.session.rollback()
        print(f"  ⚠️ Permission migration failed (non-fatal): {exc}")
