...
def login():
    """
    Login endpoint - authenticates via Authentik PostgreSQL database.
    Generates a JWT token compatible with Authentik for use in the application.
    """
    import psycopg2
    import os
    import jwt
    ...
    try:
        ...
        try:
            ...
            # Validate password using Django's PBKDF2 format
            password_valid = False

            # For development, accept "admin123" as universal password
            dev_mode = os.getenv('PROJECT_STATE', 'development') == 'development'
            if dev_mode and password == 'admin123':
                password_valid = True
                logging.info(f"Development login for user {username} with admin123")
            else:
                # Validate against stored Django password hash
                if stored_password and stored_password.startswith('pbkdf2_sha256$'):
                    try:
                        ...
                        logging.warning(f"Password hash validation error: {e}")

            if not password_valid:
                return jsonify({'error': 'Invalid credentials'}), 401

            # Get user roles from LLARS database
            ...
            try:
                ...
                logging.warning(f"Could not fetch LLARS roles for {user_username}: {e}")

            # Generate JWT token (HS256 for internal use)
            jwt_secret = os.getenv('AUTHENTIK_SECRET_KEY', 'dev-authentik-secret-change-me')
            now = datetime.datetime.now(timezone.utc)
            exp = now + datetime.timedelta(hours=8)  # 8-hour token lifetime
            ...
            logging.info(f"User {username} logged in successfully with roles: {user_roles}")
            ...
            logging.error(f"Authentik database error: {e}")
