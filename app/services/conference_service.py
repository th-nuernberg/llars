...
def _sanitize_stored_url(value):
    """
    XSS-Defense-in-depth für gespeicherte Link-Felder (overleaf/external/website).

    Conference-Daten werden in Gruppen geteilt und im Frontend als `:href`
    gerendert. Das Frontend blockt gefährliche Schemata zwar bereits beim
    Rendern (safeExternalUrl), aber die API ist direkt erreichbar — daher hier
    serverseitig nur http(s)/mailto zulassen und z.B. `javascript:`-URLs auf
    None setzen (Stored-XSS-Vektor entschärfen).
    """
    ...
    if lowered.startswith(('http://', 'https://', 'mailto:')):
        return stripped
    # Schema-loses "example.com/..." als https interpretieren (kein ":" vor erstem /)
    scheme_sep = lowered.find(':')
    slash = lowered.find('/')
    if scheme_sep == -1 or (slash != -1 and slash < scheme_sep):
        return stripped
    # Hat ein anderes Schema (javascript:, data:, vbscript: ...) → verwerfen
    return None
...
class ConferenceSeriesService:
    ...
    def create_series(data: Dict[str, Any], username: str) -> Dict[str, Any]:
        ...
        series = ConferenceSeries(
            ...
            website_url=_sanitize_stored_url(data.get("website_url")),
    ...
    def update_series(series_id: int, data: Dict[str, Any], username: str) -> Optional[Dict[str, Any]]:
        ...
        for field in ["name", "acronym", "website_url", "keywords", "notes"]:
            if field in data:
                val = _sanitize_stored_url(data[field]) if field == "website_url" else data[field]
...
class ConferenceService:
    ...
    def create_paper(data: Dict[str, Any], username: str) -> Dict[str, Any]:
        ...
        paper = Paper(
            ...
            overleaf_url=_sanitize_stored_url(data.get("overleaf_url")),
            external_url=_sanitize_stored_url(data.get("external_url")),
            keywords=data.get("keywords"),
    ...
    def update_paper(paper_id: int, data: Dict[str, Any], username: str) -> Optional[Dict[str, Any]]:
        ...
        updatable = [
            "title", "conference_id", "overleaf_url",
            "external_url", "keywords", "description", "notes",
        ]
        url_fields = {"overleaf_url", "external_url"}
        for field in updatable:
            if field in data:
                val = _sanitize_stored_url(data[field]) if field in url_fields else data[field]
