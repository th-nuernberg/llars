...
class OutputExportService:
    ...
    def export_to_csv(
        ...
        *,
        ...
        def _csv_safe(value):
            # Prevent spreadsheet formula injection: Excel/Sheets execute a cell
            # beginning with = + - @ (or a control char) as a formula. LLM
            # generated_content / error_message are untrusted, so neutralize them.
    ...
    def create_evaluation_scenario_fixed(
        ...
        *,
        ...
        # Labeling co-pilot + scenario parts: this server-side create path
        # bypasses the /api/scenarios normalize hooks, so stamp the copilot
        # salt/prompt-version and resolve part size-specs against the items
        # created above (same transaction) here as well.
        if function_type_id == 7:
            ...
            scenario.config_json = LabelingCopilotService.normalize_config_on_write(
                scenario.config_json
            )
            scenario.config_json = ScenarioPartsService.normalize_config_on_write(
                scenario.config_json
            )
