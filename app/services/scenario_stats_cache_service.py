...
def _get_db_computed_at(scenario_id: int) -> Optional[float]:
    """Cheap (single indexed column) read of the cache row's computed_at as a
    UTC epoch. Used for cross-worker in-memory coherence; returns None if there
    is no row yet or on any error (callers then keep their in-memory copy)."""
    try:
        row = (
            ScenarioStatsCache.query
            .with_entities(ScenarioStatsCache.computed_at)
            .filter_by(scenario_id=scenario_id)
            .first()
        )
        if row and row[0]:
            return row[0].replace(tzinfo=timezone.utc).timestamp()
    except Exception:
        return None
    return None
...
def get_cached_stats(scenario_id: int) -> Dict[str, Any]:
    ...
    if mem is not None:
        ...
        # Cross-worker coherence: with multiple gunicorn/gevent workers, another
        # worker may have marked this scenario dirty (mark_dirty sets computed_at
        # to year 2000) or recomputed it — this worker's in-memory copy wouldn't
        # know and would serve stale IRR/progress for up to `interval` seconds.
        # One cheap indexed timestamp read reconciles it before trusting the hit.
        db_ts = _get_db_computed_at(scenario_id)
        if db_ts is not None and abs(db_ts - ts) > 1.0:
            # DB changed since we cached (dirtied or refreshed elsewhere) -> drop
            # the local entry and fall through to the DB path below.
