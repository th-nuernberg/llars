...
def register_messaging_events(socketio):
    ...
    def handle_join_chat(data=None):
        ...
        try:
            ...
            # AuthN + AuthZ: only an active participant may join a conversation
            # room (otherwise any authenticated client could eavesdrop on any
            # private conversation by guessing its id).
            username = _require_authenticated_user()
            if not username:
                return
            from db.models.messaging import MessagingParticipant
            is_participant = MessagingParticipant.query.filter_by(
                conversation_id=conversation_id,
                username=username,
                is_active=True,
            ).first()
            if not is_participant:
                emit("messaging:error", {"error": "Forbidden"})
                return
