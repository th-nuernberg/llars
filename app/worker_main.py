...
def _run_ner_worker(worker_label: str) -> None:
    """Warm Flair NER consumer for the anonymize tool.

    Loads the 2.2GB Flair model ONCE (warm) in this dedicated worker process and
    serves NER jobs from Redis, so the gevent web tier never loads the model
    (no ~35s cold load, no per-web-worker RAM copies). See
    services/anonymize/ner_remote.py for the protocol and the web-side fallback.
    """
    from main import app
    from services.anonymize.ner_remote import run_ner_worker_loop

    with app.app_context():
        run_ner_worker_loop(worker_label)
...
def main() -> int:
    ...
    # Single NER worker by default: one warm Flair model copy (RAM-bounded).
    # Set ANONYMIZE_NER_WORKER_PROCESSES=0 to disable the offload (web falls back
    # to loading the model locally).
    ner_worker_processes = max(0, int(os.environ.get("ANONYMIZE_NER_WORKER_PROCESSES", "") or 1))
    ...
    if ner_worker_processes:
        processes.extend(_spawn_named_processes("ner", ner_worker_processes, _run_ner_worker))
