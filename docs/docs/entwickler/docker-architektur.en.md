...
```
Layer 1: System packages (~200 MB)     ← Rarely changes
Layer 2: Heavy ML packages (~4 GB)     ← torch, transformers, flair, chromadb
         (requirements-heavy.txt)         Changes very rarely
Layer 3: Python packages (~500 MB)     ← requirements.txt
         (with BuildKit cache mount)      Changes occasionally
Layer 4: Playwright Chromium (~300 MB) ← Rarely changes
Layer 5: App code (~500 MB)            ← Changes frequently
```

**Effect:** A code change only rebuilds Layer 5 (~seconds). A dependency change rebuilds Layers 3+5. The heavy ML packages stay cached.
