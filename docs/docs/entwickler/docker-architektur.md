...
```
Layer 1: System-Pakete (~200 MB)       ← Ändert sich selten
Layer 2: Schwere ML-Pakete (~4 GB)     ← torch, transformers, flair, chromadb
         (requirements-heavy.txt)         Ändert sich sehr selten
Layer 3: Python-Pakete (~500 MB)       ← requirements.txt
         (mit BuildKit Cache-Mount)       Ändert sich gelegentlich
Layer 4: Playwright Chromium (~300 MB) ← Ändert sich selten
Layer 5: App-Code (~500 MB)            ← Ändert sich häufig
```

**Effekt:** Eine Code-Änderung baut nur Layer 5 neu (~Sekunden). Eine Dependency-Änderung baut Layer 3+5 neu. Die schweren ML-Pakete bleiben gecacht.
...
|-------|-------|------------|
| Flask Backend | ~9 GB | python:3.10-slim |
