...
const sessionSyncSuspended = ref(false)
const sessionRestoreInProgress = ref(false)
...
function readWizardSession() {
  try {
    ...
    console.warn('[Wizard] Failed to read wizard session:', e)
    try {
...
function writeWizardSession(snapshot) {
  if (sessionSyncSuspended.value) return
  try {
    sessionStorage.setItem(WIZARD_SESSION_KEY, JSON.stringify(snapshot))
  } catch (e) {
    console.warn('[Wizard] Failed to persist wizard session:', e)
  }
}

function clearWizardSession() {
  sessionSyncSuspended.value = true
  try {
    sessionStorage.removeItem(WIZARD_SESSION_KEY)
  } catch {}
  setTimeout(() => {
    sessionSyncSuspended.value = false
  }, 0)
}
...
function buildWizardSessionSnapshot() {
  return {
    version: 1,
    updated_at: new Date().toISOString(),
    is_open: Boolean(sessionOpen.value),
    chatbot_id: chatbotId.value,
    crawler_job_id: crawlerJobId.value,
    collection_id: collectionId.value,
    build_status: buildStatus.value,
    current_step: currentStep.value,
    has_auto_generated_fields: Boolean(hasAutoGeneratedFields.value),
    wizard_data: wizardData.value,
    crawler_config: crawlerConfig.value
  }
}

function persistWizardSessionSnapshot() {
  if (sessionRestoreInProgress.value) return
  writeWizardSession(buildWizardSessionSnapshot())
}
...
function handleClose() {
  ...
  persistWizardSessionSnapshot()
  emit('close')
}

async function syncWizardFromBackend(id) {
  try {
    ...
    if (chatbot) {
      // Only fill blanks to avoid overriding unsaved wizard edits.
      if (!wizardData.value.url && chatbot.source_url) wizardData.value.url = chatbot.source_url
      if (!wizardData.value.name && chatbot.name) wizardData.value.name = chatbot.name
      if (!wizardData.value.displayName && chatbot.display_name) wizardData.value.displayName = chatbot.display_name
      if (!wizardData.value.welcomeMessage && chatbot.welcome_message) wizardData.value.welcomeMessage = chatbot.welcome_message
      if (!wizardData.value.fallbackMessage && chatbot.fallback_message) wizardData.value.fallbackMessage = chatbot.fallback_message
      if (!wizardData.value.modelName && chatbot.model_name) wizardData.value.modelName = chatbot.model_name
...
async function restoreWizardSession() {
  ...
  try {
    ...
    if (saved.wizard_data) {
      wizardData.value = { ...wizardData.value, ...saved.wizard_data }
    }
    if (saved.crawler_config) {
      crawlerConfig.value = { ...crawlerConfig.value, ...saved.crawler_config }
    }
    if (saved.chatbot_id) {
      setChatbotId(saved.chatbot_id)
    }
    if (saved.collection_id) {
      setCollectionId(saved.collection_id)
    }
    if (saved.crawler_job_id) {
      setCrawlerJobId(saved.crawler_job_id)
    }
    if (saved.build_status) {
      setStatus(saved.build_status)
    }
    if (saved.current_step) {
      navigateToStep(saved.current_step)
    }
    if (saved.has_auto_generated_fields !== undefined) {
      hasAutoGeneratedFields.value = Boolean(saved.has_auto_generated_fields)
    }
  } finally {
    sessionSyncSuspended.value = false
  ...
  // Sync in-memory state with backend progress/status
  if (chatbotId.value) {
    await syncWizardFromBackend(chatbotId.value)
  }

  // (Re-)subscribe to progress updates
...
watch(wizardData, persistWizardSessionSnapshot, { deep: true })
watch(crawlerConfig, persistWizardSessionSnapshot, { deep: true })
watch(hasAutoGeneratedFields, persistWizardSessionSnapshot)
watch(sessionOpen, persistWizardSessionSnapshot)
</script>
