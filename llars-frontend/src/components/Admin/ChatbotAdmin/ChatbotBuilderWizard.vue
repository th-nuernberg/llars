...
function buildWizardSessionSnapshot() {
  return {
    ...
    // Track which fields were being generated (for recovery after navigation)
    generating_fields: { ...generating.value }
  }
}
...
async function restoreWizardSession({ targetChatbotId = null } = {}) {
  const saved = readWizardSession()
  if (!saved) return
  if (targetChatbotId && saved.chatbot_id && saved.chatbot_id !== targetChatbotId) return
  if (targetChatbotId && !saved.chatbot_id) return
  ...
  try {
    ...
    // Sync from the global field generation service
    // If any streams were running, they'll still be running and we'll get the latest content
    // The service persists across component lifecycle, so we just sync the current state
  } finally {
    sessionSyncSuspended.value = false
...
watch(generating, persistWizardSessionSnapshot, { deep: true })
</script>
