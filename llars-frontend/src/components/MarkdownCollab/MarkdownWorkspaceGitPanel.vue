...
// ---------------------------------------------------------------------------
// Socket handling - Live-Refresh bei Commits anderer Nutzer
//
// Das Markdown-Collab-Backend kennt nur *Document*-Rooms
// (`markdown_collab:subscribe_document`), keinen Workspace-Room fuer Commits -
// `emit_commit_created()` sendet pro Dokument (siehe
// app/socketio_handlers/events_markdown_collab.py). Deshalb abonniert dieses
// Workspace-Panel den Room des aktuell geoeffneten Dokuments. Commits in
// anderen Dateien des Workspaces holt der bestehende REST-Refresh nach
// (checkForChanges/loadRecentCommits beim Dokumentwechsel bzw. nach eigenem
// Commit) - bewusste Einschraenkung statt eines zusaetzlichen Backend-Rooms.
// ---------------------------------------------------------------------------
let socket = null
let onSocketConnect = null
let subscribedDocumentId = null
...
function subscribeDocument(documentId) {
  ...
  subscribedDocumentId = documentId
}

function unsubscribeDocument() {
  if (!socket || !subscribedDocumentId) return
  socket.emit('markdown_collab:unsubscribe_document', { document_id: subscribedDocumentId })
  subscribedDocumentId = null
}
...
function setupSocket() {
  ...
  onSocketConnect = () => {
    // Nach (Re-)Connect ist der Room-Beitritt verloren -> neu abonnieren
    subscribedDocumentId = null
    subscribeDocument(props.selectedDocumentId)
  }
...
// Watch for document changes to refresh diff + move the socket subscription
// to the newly selected document (see socket handling comment above).
watch(() => props.selectedDocumentId, async (newId, oldId) => {
  if (newId !== oldId) {
    unsubscribeDocument()
    subscribeDocument(newId)
    resetDiffState()
