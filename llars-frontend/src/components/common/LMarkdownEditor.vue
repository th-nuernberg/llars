...
// Public API: insert arbitrary text at the current cursor position (or at end).
// Used by parent components that place variable-picker chips next to this editor.
...
/* Phones only: stack editor above preview. Tablets keep the
   side-by-side split (breakpoint lowered from 960px → 600px). */
@media (max-width: 600px) {
  .markdown-body {
    grid-template-columns: 1fr;
  }

  /* Shrink the textarea on phones (~4 rows instead of the default ~8).
     auto-grow still expands as the user types; this only caps the
     initial / empty height to save vertical space. */
  .markdown-input :deep(textarea) {
    min-height: 6.5em;
  }
}
