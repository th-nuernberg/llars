      ...
      <div class="threads-grid">
        ...
        <template v-else>
          <div
            ...
            :key="session.id"
            class="thread-card"
            :class="{ 'is-done': session.status === 'completed' }"
...
const { isLoading, withLoading } = useSkeletonLoading(['sessions'])

const doneCount = computed(() => sessions.value.filter(s => s.status === 'completed').length)

onMounted(async () => {
  await withLoading('sessions', async () => {
    try {
...
function getStatus(session: any): string {
  ...
  if (session.status === 'progressing') return 'in_progress'
  return 'pending'
}
