/**
 * Substitute ``{{key}}`` placeholders in a markdown briefing string
 * with values from a per-item metadata dict. LocalizedString values
 * (``{de, en}``) are unwrapped to the current locale so authors can
 * write ``{{channel_label}}`` and get a clean German/English string
 * instead of ``[object Object]``.
 *
 * Unknown keys are left untouched (rendered verbatim as
 * ``{{key}}``) so authors notice typos in their template.
 */
export function substituteBriefingVariables(template, meta, locale = 'de') {
  if (!template || !meta) return template || ''
  return String(template).replace(/\{\{(\w+)\}\}/g, (_, key) => {
    const v = meta[key]
    if (v === undefined || v === null) return `{{${key}}}`
    if (typeof v === 'object' && !Array.isArray(v)) {
      return String(v[locale] ?? v.de ?? v.en ?? JSON.stringify(v))
    }
    return String(v)
  })
}
