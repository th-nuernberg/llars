...
describe('useSourcePanel', () => {
  ...
  describe('loadPanelScreenshot', () => {
    ...
    it('SRC_PNL_021b: skips loading when source has no screenshot_url (text/docs source)', async () => {
      // A source with only a document_id (every RAG source has one) must NOT
      // trigger a screenshot fetch — text/docs sources have no screenshot and
      // the old document_id fallback 404'd ("Konnte Screenshot nicht laden").
      panel.sourcePanel.value.source = { document_id: 'doc-1' }

      await panel.loadPanelScreenshot()

      expect(axios.get).not.toHaveBeenCalled()
      expect(panel.sourcePanel.value.screenshotError).toBe(null)
    })
