...
describe('useComparisonEvaluation Composable', () => {
  ...
  // ==================== Rater notes ====================
  //
  // The note is study data: a borderline-case remark, a rationale. It used to
  // be collected by the dialog and then silently dropped, because
  // saveMetadata() was an empty stub whose comment claimed the note "is saved
  // with the option selection" — true only for a note typed BEFORE choosing.

  describe('Rater notes', () => {
    it('COMP_EVAL_060: a note edited after the choice is persisted', async () => {
      const scenarioId = ref(1)
      const comp = useComparisonEvaluation(scenarioId)
      comp.currentItem.value = { item_id: 10, thread_id: 10 }
      comp.items.value = [{ thread_id: 10, evaluated: false }]
      ...
      comp.notes.value = 'Grenzfall, beide gleichwertig'
      comp.saveMetadata()
      ...
      expect(axios.post).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          function_type: 'comparison',
          choice: 'A',
          notes: 'Grenzfall, beide gleichwertig'
        })
      )
    })

    it('COMP_EVAL_061: the existing choice is re-sent, not overwritten', async () => {
      // choice is NOT NULL server-side, so a note cannot be stored on its own.
      // Re-sending the wrong choice would rewrite the rater's judgement.
      const scenarioId = ref(1)
      const comp = useComparisonEvaluation(scenarioId)
      comp.currentItem.value = { item_id: 10, thread_id: 10 }
      comp.items.value = [{ thread_id: 10, evaluated: false }]
      ...
      await comp.selectOption('tie')
      axios.post.mockClear()

      comp.notes.value = 'gleichwertig'
      comp.saveMetadata()
      await vi.advanceTimersByTimeAsync(1000)

      expect(axios.post.mock.calls[0][1].choice).toBe('tie')
    })

    it('COMP_EVAL_062: without a choice nothing is sent', async () => {
      // The backend would reject it (choice NOT NULL). The text stays in memory
      // and rides along with the upcoming selectOption().
      const scenarioId = ref(1)
      const comp = useComparisonEvaluation(scenarioId)
      comp.currentItem.value = { item_id: 10, thread_id: 10 }
      axios.post.mockResolvedValue({ data: {} })

      comp.notes.value = 'gleich'
      comp.saveMetadata()
      await vi.advanceTimersByTimeAsync(1000)

      expect(axios.post).not.toHaveBeenCalled()

      // ...but it is not lost: choosing afterwards carries it.
      await comp.selectOption('tie')
      expect(axios.post.mock.calls[0][1].notes).toBe('gleich')
    })
  })
})
