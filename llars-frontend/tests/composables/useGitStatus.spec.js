...
describe('useGitStatus', () => {
  ...
  describe('Helper Functions', () => {
    it('GIT_STAT_019: getFileIcon returns correct icon for .md', () => {
      const { getFileIcon } = useGitStatus(ref(1), { autoSetup: false })
      expect(getFileIcon('document.md')).toBe('mdi-language-markdown')
    })

    it('GIT_STAT_020: getFileIcon returns correct icon for .json', () => {
      const { getFileIcon } = useGitStatus(ref(1), { autoSetup: false })
      expect(getFileIcon('data.json')).toBe('mdi-code-json')
    })
    ...
    it('GIT_STAT_022: getFileIconColor returns primary for .md', () => {
      const { getFileIconColor } = useGitStatus(ref(1), { autoSetup: false })
      expect(getFileIconColor('document.md')).toBe('primary')
    })

    it('GIT_STAT_023: getFileIconColor returns orange for .json', () => {
      const { getFileIconColor } = useGitStatus(ref(1), { autoSetup: false })
      expect(getFileIconColor('data.json')).toBe('orange')
    })

    it('GIT_STAT_024: getFileIcon returns document icon for .txt', () => {
      const { getFileIcon } = useGitStatus(ref(1), { autoSetup: false })
      expect(getFileIcon('notes.txt')).toBe('mdi-file-document')
    })

    it('GIT_STAT_025: getFileIconColor returns grey for unknown extension', () => {
      const { getFileIconColor } = useGitStatus(ref(1), { autoSetup: false })
      expect(getFileIconColor('file.xyz')).toBe('grey')
    })
