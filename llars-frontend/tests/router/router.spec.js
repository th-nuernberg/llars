...
describe('Router Configuration', () => {
  ...
  describe('route definitions', () => {
    it('ROUTER_001: root renders the public landing page natively', () => {
      const root = router.getRoutes().find(r => r.path === '/')
      expect(root).toBeDefined()
      // Seit der Landing-Page-Finalisierung rendert "/" die LandingPage
      // direkt (kein Login-Redirect mehr). Eingeloggte User werden über
      // hiddenWhenAuth weiter auf /Home geleitet; hideAppChrome versteckt
      // AppBar + globalen Footer (eigene Nav/Footer der Landing Page).
      expect(root.redirect).toBeUndefined()
      expect(root.name).toBe('LandingPage')
      expect(root.meta.requiresAuth).toBe(false)
      expect(root.meta.hiddenWhenAuth).toBe(true)
      expect(root.meta.hideAppChrome).toBe(true)
    })

    it('ROUTER_001b: /landing stays as alias redirect to root', () => {
      const alias = router.getRoutes().find(r => r.path === '/landing')
