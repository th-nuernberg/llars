...
describe('EVAL_TYPES', () => {
  ...
  it('PRESET_002: has exactly 8 types', () => {
    // 6 generic + COMMUNICATION_COMPARISON (8) + CONVERSATION_LABELING (9)
    expect(Object.keys(EVAL_TYPES)).toHaveLength(8)
  })
})
...
describe('getTypesByCategory', () => {
  it('PRESET_082: returns general and llars categories', () => {
    ...
    // 2 long-standing LLARS types (mail_rating, authenticity) plus the two
    // newer specialisations: communication_comparison (8, counselling-style
    // A/B) and conversation_labeling (9, span-level labeling in context).
    expect(cats.llars).toHaveLength(4)
    expect(cats.general).toContain(EVAL_TYPES.RATING)
    expect(cats.llars).toContain(EVAL_TYPES.MAIL_RATING)
    expect(cats.llars).toContain(EVAL_TYPES.COMMUNICATION_COMPARISON)
    expect(cats.llars).toContain(EVAL_TYPES.CONVERSATION_LABELING)
  })
})
...
describe('getLlarsTypes', () => {
  it('PRESET_084: returns the 4 LLARS-specific types', () => {
