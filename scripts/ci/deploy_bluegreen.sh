...
# Matomo laeuft als geteilter Service und ist NICHT Teil des Blue-Green-
# Farbwechsels. Aenderungen an init-matomo.sh / config.ini.php / Branding-Logo
# greifen daher nicht automatisch beim Deploy. Diese Funktion recreated
# matomo-init (idempotent) einmal, damit z.B. assume_secure_protocol und das
# LLARS-Custom-Logo angewandt werden. Bewusst NON-FATAL: Matomo ist nicht
# deploy-kritisch, ein Fehler hier darf den bereits erfolgten Switch nicht kippen.
ensure_matomo_config() {
  if ! docker inspect llars_matomo_db_service >/dev/null 2>&1; then
    echo "matomo-db not present; skipping Matomo config step."
    return 0
  fi
  echo "Recreating matomo-init to apply Matomo config/branding (idempotent)..."
  docker compose -f docker-compose.yml -f docker-compose.prod.yml up -d --force-recreate matomo-init \
    || echo "WARN: matomo-init recreate failed (non-fatal)."
}
...
cmd_deploy() {
  ...
  # -----------------------------------------------------------------------
  # Refresh the MkDocs static site so documentation changes go live.
  # mkdocs-service builds the static site ONCE at container start
  # (PROJECT_STATE=production → `mkdocs build` then a plain http.server), so a
  # deploy that only updates ./docs source files would otherwise keep serving
  # the stale site. It is a standalone container (NOT part of the blue-green
  # color cycle), so restart it by name to trigger a rebuild against the freshly
  # pulled ./docs. Non-fatal: a docs rebuild failure must not block the app deploy.
  # -----------------------------------------------------------------------
  if docker ps -a --format '{{.Names}}' | grep -qx 'llars_mkdocs_service'; then
    echo "Rebuilding MkDocs documentation (restart llars_mkdocs_service)..."
    docker restart llars_mkdocs_service >/dev/null 2>&1 \
      && echo "  ✓ MkDocs rebuild triggered" \
      || echo "  ⚠️ MkDocs restart failed (non-fatal — docs may be stale until next restart)"
  fi
...
cmd_switch() {
  ...
  echo ""
  echo "Applying shared Matomo config (branding, assume_secure_protocol)..."
  ensure_matomo_config
