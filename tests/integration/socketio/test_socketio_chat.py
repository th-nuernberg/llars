...
class TestPromptStreamHandler:
    ...
    def _auth_ok(self):
        """Bypass the socket auth/permission/model-access/rate-limit gate so these
        tests exercise the LLM streaming pipeline. The gate itself (added in the
        2026-06-10 pentest fix) has its own dedicated coverage; here we mock it to
        a permitted user. The handler uses function-local imports, so we patch the
        source modules (looked up at call time)."""
        user = Mock()
        user.username = 'tester'
        with patch('socketio_handlers.socket_auth.socket_user', return_value=user), \
             patch('services.permission_service.PermissionService.check_permission', return_value=True), \
             patch('services.llm.llm_access_service.LLMAccessService.user_can_access_model', return_value=True), \
             patch('socketio_handlers.socket_rate_limit.allow_llm_event', return_value=True):
            yield
