...
class TestUploadDocument:
    ...
    def test_ROUTE_RAG_DOC_UPLOAD_004_foreign_collection_forbidden(
        self, auth_researcher, real_app
    ):
        """Audit finding C: a user holding the global feature:rag:edit permission
        but WITHOUT edit rights on the *target* collection must be blocked (403)
        from uploading into it — otherwise any editor could poison a foreign
        collection whose chatbot then retrieves and cites the injected document.
        """
        import io
        from db.models.permission import Role, Permission, RolePermission

        with real_app.app_context():
            # Grant global feature:rag:edit to researcher so the request passes
            # @require_permission and reaches the per-collection ownership gate.
            role = Role.query.filter_by(role_name='researcher').first()
            perm = Permission.query.filter_by(permission_key='feature:rag:edit').first()
            assert role and perm
            if not RolePermission.query.filter_by(role_id=role.id, permission_id=perm.id).first():
                real_app.db.session.add(RolePermission(role_id=role.id, permission_id=perm.id))
                real_app.db.session.commit()
            # Collection owned by 'admin' — researcher is neither owner nor admin.
            coll = _create_collection_in_db(real_app.db, name='victim-coll',
                                            display_name='Victim Coll', created_by='admin')
            coll_id = coll.id

        response = auth_researcher.post(
            '/api/rag/documents/upload',
            data={'file': (io.BytesIO(b'poison content'), 'poison.txt'),
                  'collection_id': str(coll_id)},
            content_type='multipart/form-data',
        )
        # With rag:edit granted and a file present, a 403 can only originate from
        # the per-collection edit gate added for finding C.
        assert response.status_code == 403
