...
class TestLabelingLabelResolver:
    """[SCEN_LABEL_*] _labeling_label_resolver maps stored category_id -> display
    name across config shapes — the data-tab detail dialog relies on this so a
    human labeling shows 'include', not the opaque 'cat_…' id."""

    def _resolver(self):
        from routes.scenarios.scenario_manager_api import _labeling_label_resolver
        return _labeling_label_resolver

    def test_SCEN_LABEL_001_nested_api_v1_config(self):
        """[SCEN_LABEL_001] api_v1 nesting (eval_config.config) + localized names."""
        cfg = {"eval_config": {"config": {
            "categories": [
                {"id": "cat_1", "name": {"de": "include", "en": "include"}},
                {"id": "cat_2", "name": {"de": "exclude", "en": "exclude"}},
            ],
            "unsureOption": {"id": "unsure", "name": {"de": "uncertain", "en": "uncertain"}},
        }}}
        resolve = self._resolver()(cfg)
        assert resolve("cat_1") == "include"
        assert resolve("cat_2") == "exclude"
        assert resolve(None, is_unsure=True) == "uncertain"

    def test_SCEN_LABEL_002_flat_config_plain_names(self):
        """[SCEN_LABEL_002] manual/flat config with plain-string names."""
        cfg = {"categories": [{"id": "pos", "name": "Positive"}, {"id": "neg", "name": "Negative"}]}
        resolve = self._resolver()(cfg)
        assert resolve("pos") == "Positive"
        assert resolve("neg") == "Negative"

    def test_SCEN_LABEL_003_labels_key_and_json_string(self):
        """[SCEN_LABEL_003] `labels` key + config passed as a JSON string."""
        cfg = json.dumps({"config": {"labels": [{"id": "a", "label": {"de": "A-Label"}}]}})
        resolve = self._resolver()(cfg)
        assert resolve("a") == "A-Label"

    def test_SCEN_LABEL_004_unknown_id_falls_back_to_id(self):
        """[SCEN_LABEL_004] Unknown / empty config returns the id unchanged."""
        resolve = self._resolver()({})
        assert resolve("cat_unknown") == "cat_unknown"
