...
class TestGetDefaultConfigJson:
    ...
    def test_EXPORT_SVC_031_conversation_labeling_default_config(self):
        """Type 9 needs a usable default config like every other type.

        Without it the config editor opens empty and a scenario created through
        the manager has no labels at all.
        """
        from services.evaluation.schema_export_service import SchemaExportService

        config = SchemaExportService.get_default_config_json('conversation_labeling')
        assert config['categories'], "conversation labeling needs default categories"
        # The visibility fields are what separate it from classic labeling;
        # hiding later messages is the load-bearing one (raters must not know
        # how the conversation ends).
        assert config['no_future_messages'] is True
        assert config['context_window'] == 3

    def test_EXPORT_SVC_032_conversation_labeling_is_not_wizard_inferrable(self):
        """The wizard infers a type from an uploaded table. Conversation
        labeling cannot be inferred — it needs a segmentation only the import
        supplies — so it must stay out of the detection examples, or the AI
        would recommend a scenario with no spans and nothing to decide."""
        from services.evaluation.schema_export_service import SchemaExportService

        assert 'conversation_labeling' not in SchemaExportService.INPUT_DATA_EXAMPLES
