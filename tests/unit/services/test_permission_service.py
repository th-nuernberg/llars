...
class TestCheckPermissionEdgeCases:
    ...
    def test_PERM_SVC_032_two_roles_granting_same_permission(self, app, db, app_context):
        """[PERM_SVC-032] Multi-role user, overlapping grant → True, no crash.

        Regression for the 2026-08-17 prod incident: IJCAI demo accounts hold
        evaluator AND ijcai_reviewer, both granting e.g. feature:rating:view.
        The role-permission existence query matched two rows and
        scalar_one_or_none raised MultipleResultsFound → 500 on every
        permission-gated route for such users.
        """
        from services.permission_service import PermissionService

        _create_user(db, 'multi_role_032')
        _assign_role(db, 'multi_role_032', 'evaluator')
        _assign_role(db, 'multi_role_032', 'researcher')  # both grant rating:view

        assert PermissionService.check_permission(
            'multi_role_032', 'feature:rating:view'
        ) is True

    def test_PERM_SVC_033_user_has_role_with_multiple_matches(self, app, db, app_context):
        """[PERM_SVC-033] user_has_role survives a user holding several of the
        requested roles (same MultipleResultsFound failure mode as PERM_SVC-032
        via the evaluator→viewer alias expansion)."""
        from services.permission_service import PermissionService

        _create_user(db, 'multi_role_033')
        _assign_role(db, 'multi_role_033', 'evaluator')
        _assign_role(db, 'multi_role_033', 'viewer')

        assert PermissionService.user_has_role('multi_role_033', 'evaluator') is True
