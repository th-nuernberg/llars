...
class TestLinkManagement:
    ...
    def test_REF_021_update_link(self, app, db, app_context):
        ...
        assert updated.role_name == 'researcher'
        assert updated.max_uses == 10

    def test_REF_021b_referral_link_cannot_grant_admin(self, app, db, app_context):
        """[REF-021b] SECURITY: referral links must not be settable to 'admin'."""
        from services.referral_service import ReferralService
        from decorators.error_handler import ValidationError

        campaign = self._create_campaign(ReferralService)

        # Cannot create an admin-granting link.
        with pytest.raises(ValidationError):
            ReferralService.create_link(
                campaign_id=campaign.id, created_by='admin', role_name='admin'
            )

        # Cannot update an existing link into an admin-granting one either.
        link = ReferralService.create_link(
            campaign_id=campaign.id, created_by='admin', role_name='evaluator'
        )
        with pytest.raises(ValidationError):
            ReferralService.update_link(link.id, role_name='admin')
...
class TestLinkValidation:
    ...
    def _create_active_campaign_with_link(self, ReferralService):
        ...
        link = ReferralService.create_link(
            campaign_id=campaign.id,
            created_by='admin',
            # Explicit allowlisted role — these validation tests mock get_setting
            # globally, which would otherwise make get_default_role() return the
            # mock value and trip the role allowlist.
            role_name='evaluator',
            slug='valid-link',
        )
...
class TestEnrollAndRedeemHelpers:
    """Tests for the shared enroll helper + idempotent register_user.

    These back the authenticated POST /api/referral/redeem flow, which reuses
    ReferralService.enroll_user_in_scenario and ReferralService.register_user.
    """
    ...
    def test_REF_050_enroll_no_target_scenario_is_noop(self, app, db, app_context):
        """[REF-050] Link without target scenario → no enrollment."""
        from services.referral_service import ReferralService
        user = self._make_user(db, 'enr_user_0')
        campaign = ReferralService.create_campaign(name='C0', created_by='admin')
        link = ReferralService.create_link(campaign_id=campaign.id, created_by='admin')

        enrolled, already = ReferralService.enroll_user_in_scenario(user, link)
        assert enrolled is False
        assert already is False

    def test_REF_051_enroll_creates_assessor_membership(self, app, db, app_context):
        """[REF-051] Enroll into target scenario creates an ACTIVE assessor row."""
        from services.referral_service import ReferralService
        from db.models.scenario import ScenarioUsers, MembershipStatus
        user = self._make_user(db, 'enr_user_1')
        scenario = self._make_scenario(db)
        campaign = ReferralService.create_campaign(name='C1', created_by='admin')
        link = ReferralService.create_link(
            campaign_id=campaign.id, created_by='admin', target_scenario_id=scenario.id
        )
        ...
        row = ScenarioUsers.query.filter_by(scenario_id=scenario.id, user_id=user.id).first()
        assert row is not None
        assert row.evaluation_role == 'assessor'
        assert row.is_assessor is True
        assert row.membership_status == MembershipStatus.ACTIVE

    def test_REF_052_enroll_is_idempotent(self, app, db, app_context):
        """[REF-052] A second enroll is a no-op (already_enrolled)."""
        from services.referral_service import ReferralService
        from db.models.scenario import ScenarioUsers
        user = self._make_user(db, 'enr_user_2')
        scenario = self._make_scenario(db)
        campaign = ReferralService.create_campaign(name='C2', created_by='admin')
        link = ReferralService.create_link(
            campaign_id=campaign.id, created_by='admin', target_scenario_id=scenario.id
        )

        ReferralService.enroll_user_in_scenario(user, link)
        enrolled, already = ReferralService.enroll_user_in_scenario(user, link)
        assert enrolled is False
    ...
    def test_REF_053_enroll_does_not_change_existing_role(self, app, db, app_context):
        """[REF-053] An existing membership's role is left untouched on re-enroll."""
        from services.referral_service import ReferralService
        from db.models.scenario import (
            ScenarioUsers, MembershipStatus, ScenarioRoles,
        )
        user = self._make_user(db, 'enr_user_3')
        scenario = self._make_scenario(db)
        # Pre-existing OWNER/editor membership (e.g. a researcher who owns it).
        pre = ScenarioUsers(
            scenario_id=scenario.id, user_id=user.id,
        ...
        campaign = ReferralService.create_campaign(name='C3', created_by='admin')
        link = ReferralService.create_link(
            campaign_id=campaign.id, created_by='admin', target_scenario_id=scenario.id
        )
        enrolled, already = ReferralService.enroll_user_in_scenario(user, link)
        ...
        row = ScenarioUsers.query.filter_by(scenario_id=scenario.id, user_id=user.id).first()
        assert row.manager_role == 'owner'  # NOT downgraded to assessor
        assert row.evaluation_role == 'none'

    def test_REF_054_register_user_idempotent_reattributes(self, app, db, app_context):
        """[REF-054] register_user re-attributes an existing username (no duplicate)."""
        from services.referral_service import ReferralService
        from db.models.referral import ReferralRegistration
        campaign = ReferralService.create_campaign(name='C4', created_by='admin')
        link1 = ReferralService.create_link(campaign_id=campaign.id, created_by='admin')
        link2 = ReferralService.create_link(campaign_id=campaign.id, created_by='admin')

        ReferralService.register_user(link=link1, username='dup_user')
        reg = ReferralService.register_user(link=link2, username='dup_user')

        assert reg.link_id == link2.id
        assert ReferralRegistration.query.filter_by(username='dup_user').count() == 1
